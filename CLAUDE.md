...
- [x] **Phase 6** — Search API + UI (`app/api/search`, masonry grid — mockups matched, iterated)
- [x] **Phase 7** — Polish + launch (SEO, Analytics, GitHub Actions cron)
- [~] **Phase 8** — Scale sources (5 added: Cool Hunting, Gessato, Fast Company Design, Hypebeast Design, Gear Patrol; ~10 more possible)
...
- **Session 2 (2026-07-21) cont'd:** Phase 6 complete + partial Phase 8. Backend: `lib/search.ts` + `app/api/search/route.ts` + `supabase/migrations/0002_search_function.sql` implement hybrid pgvector cosine + Postgres FTS ranking (weights 0.7 vector, 0.3 text, +0.15 for items in both). UI: full-bleed Pinterest-style masonry (5-6 columns, gap-2, rounded-2xl), warm-ivory palette (`app/globals.css`), sticky header with neutral pill search, item detail page at `/items/[id]` with "View original article" CTA. Landing (`app/page.tsx`) does browse-latest on empty state, hybrid search when `?q=` is set. **CLIP query template:** wrap queries as "a product design photograph of {query}" before `embedText` — measurably improves recall on physical-property queries. **Corpus swap** via migration 0003 replaced the firehose feeds with product-only feeds; added 5 more editorial product sources (Cool Hunting, Gessato, Fast Company Design, Hypebeast Design, Gear Patrol) to bring the corpus to ~250 items. **shadcn/ui was skipped** — for two components (input + card) it would have added components.json + radix packages and clashed with the palette in globals.css.
- **Session 2 (2026-07-21) cont'd:** Phase 7 complete. Sources modal (`components/sources-dialog.tsx`) uses native `<dialog>` — accessible, zero deps. `components/site-footer.tsx` shows the corpus stat + a "View sources" trigger; mounted in `app/layout.tsx` so it renders on every page. SEO metadata expanded (OG, Twitter card, theme color, title template). `@vercel/analytics` wired via `<Analytics />` — no-op locally, active on Vercel. Custom favicon at `app/icon.svg` (terracotta 4-point sparkle on warm ivory). GitHub Actions cron at `.github/workflows/ingest.yml` runs seed → ingest → download → embed daily at 03:00 UTC. **Deploy checklist:** push to GitHub, add 8 secrets to repo Actions settings (SUPABASE_URL, SUPABASE_SECRET_KEY, R2_*, R2_PUBLIC_URL, REPLICATE_API_TOKEN), connect repo to Vercel, add same secrets to Vercel env, deploy.
